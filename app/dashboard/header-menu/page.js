...
import { deleteHMenu, getHMenu, updateHMenu } from "@/app/services/authService";
...
export default function MenuList() {
  ...
  const fetchMenus = async () => {
    setLoading(true);
    try {
      const res = await getHMenu("/headermenu");
      setMenus(res);
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => {
    ...
    const handleEdit = (id) => {
    if (id) {
      router.push(`/dashboard/edit-headermenu?id=${id}`);
    } else {
  ...
  const deleteMenuApiCall = async (id) => {
    setLoading(true);
    try {
      await deleteHMenu(`/headermenu/${id}`); 
      await fetchMenus();
  ...
  const deleteApiCall = async (parentId, childIndex) => {
    ...
    try {
      ...
      await updateHMenu(`/headermenu/${parentId}`, { childmenu: updatedChild });
      await fetchMenus();
        ...
        <Div className="flex flex-wrap -mx-[10px]">
            <Div className="flex-none w-full p-[7px_10px]">
                <Div className="flex flex-wrap items-center justify-between bg-white border-l-[4px] border-l-[#294662] rounded-[5px] shadow-[0_0_50px_#00000008] p-[15px]">
                    <h4 className='font-bold leading-[1.2] mb-0 text-[20px]'>Header Menu List</h4>
                </Div>
            </Div>
        </Div>
    </Div>
    <Div className="p-[6px_16px_0_16px]">
      ...
      {menus.map((menu) => (
        <Div key={menu._id} className="bg-[#f0f0f0] mb-[5px] p-[5px_7px] text-[14px] flex w-full flex-wrap">
            ...
            <Div>{menu.itemname} <b>-- Slug:</b> {menu.slug} <b>-- At Home :</b> {menu.isStatus ? 'Active' : 'Inactive'} <b>-- Active:</b> {menu.isActive ? 'Active' : 'Inactive'} <b>-- Internal link:</b> {menu.isExternal ? 'Yes' : 'No'} <b>-- Order:</b> {menu.orderNum}
            </Div>
