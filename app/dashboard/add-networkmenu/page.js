"use client";
import { createFooterCat, createNetworkMenu } from "@/app/services/authService";
import React, { useState } from "react";
import Div from "../components/division";
import Toast from "../components/toast";

export default function AddNetworkMenu() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
       await createNetworkMenu("/network-menu", payload);
        setToast({ message: 'Network Menu Added successfully done!', type: 'success', visible: true });
        setItemname("");
        setSlug("");
        setIsActive(false);
        setIsExternal(false);
        ...
        <Div className="flex flex-wrap -mx-[10px]">
            <Div className="flex-none w-full p-[7px_10px]">
                <Div className="flex flex-wrap items-center justify-between bg-white border-l-[4px] border-l-[#294662] rounded-[5px] shadow-[0_0_50px_#00000008] p-[15px]">
                    <h4 className='font-bold leading-[1.2] mb-0 text-[20px]'>Add Network Menu</h4>
                </Div>
            </Div>
        </Div>
    </Div>
    <Div className="p-[6px_16px_0_16px]">
      ...
      <form onSubmit={handleSubmit} className="bg-white w-full min-h-[calc(100vh-220px)]">
        ...
        <Div className="w-full bg-white border-t border-[#dddddd] pt-[14px] pr-[20px] pb-[20px] text-right">
          ...
          >
            {loading ? "Saving..." : "Save Network Menu"}
          </button>
        </Div>
      </form>
    </Div>
