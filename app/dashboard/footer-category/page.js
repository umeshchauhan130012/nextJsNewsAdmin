...
import { deleteFooterCat, getFooterCat } from "@/app/services/authService";
...
export default function FooterCategoryList() {
  ...
  const fetchMenus = async () => {
    setLoading(true);
    try {
      const res = await getFooterCat("/footercategory");
      setMenus(res);
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => {
    ...
    const handleEdit = (id) => {
    if (id) {
      router.push(`/dashboard/edit-footercategory?id=${id}`);
    } else {
  ...
  const deleteMenuApiCall = async (id) => {
    setLoading(true);
    try {
      await deleteFooterCat(`/footercategory/${id}`); 
      await fetchMenus();
        ...
        <Div className="flex flex-wrap -mx-[10px]">
            <Div className="flex-none w-full p-[7px_10px]">
                <Div className="flex flex-wrap items-center justify-between bg-white border-l-[4px] border-l-[#294662] rounded-[5px] shadow-[0_0_50px_#00000008] p-[15px]">
                    <h4 className='font-bold leading-[1.2] mb-0 text-[20px]'>Footer Category List</h4>
                </Div>
            </Div>
        </Div>
    </Div>
