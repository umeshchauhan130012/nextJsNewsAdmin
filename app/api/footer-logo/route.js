...
export async function POST(req) {
  try {
    ...
    // Save Story to DB
    const footerLogo = new FooterLogo({
      logolink,
      title,
      filefirst: {
    ...
    return NextResponse.json(
      { message: "Footer Logo created successfully", footerLogo },
      { status: 201 }
    );
  } catch (error) {
...
export async function GET() {
  try {
    await dbConnect();
    const footerlogo = await FooterLogo.find();
    return NextResponse.json({ success: true, count: footerlogo.length, footerlogo });
  } catch (error) {
